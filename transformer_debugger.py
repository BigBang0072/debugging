...
class SimpleNBOW(keras.Model):
    ...
    def train_step_mouli(self,dataset_batch,task,inv_idx=None,te_tidx=None,cf_tidx=None,bidx=None,
                        ...
                        all_env_mask=None,dummy_w=None):
        '''
                ...
                for env_idx in [True,False]:
                    with tf.GradientTape() as tape2:
                        ...
                        if attn_mask_train!=None:
                            env_attn_mask_train = attn_mask_train[curr_env_mask_train]
                        env_label_train = label_train[curr_env_mask_train]
                        ...
                        #Getting the main task loss
                        main_task_prob = self.get_main_task_pred_prob(input_enc)
                        main_task_prob = dummy_w*main_task_prob
    ...
    def get_all_classifier_accuracy(self,):
        ...
        #Adding the stage2 metrics for main classifier (training)
        classifier_accuracy["main_xent"]=float(self.main_pred_xentropy.result().numpy())
        #Xentropy on the random label (mouli) to measure the model's complexity (training)
        classifier_accuracy["main_random_xent"]=float(self.main_random_pred_xentropy.result().numpy())
...
def nbow_mouli_stage1_trainer(data_args,model_args):
    ...
    #Now we will train all possible invariant models which atleast 1 invariance 
    for sidx in range(1,len(topic_list)+1):
        for topic_subset in combinations(topic_list,sidx):
            print("\n\n\n\nMerging the dataset for training invariant model")
            #We will merge the dataset if the we are creating invariance wrt to multiple topics
            ...
            cad_best_valid_main_acc,cad_best_valid_main_smin_acc,cad_best_valid_main_pdelta = \
                                        _get_prediction_from_erm(
                                                                data_args=data_args,
                                                                model_args=model_args,
                                                                data_handler=data_handler,
            ...
            pred_tv_data = dict(
                            ...
                            best_valid_main_pdelta=cad_best_valid_main_pdelta,
                            best_train_main_random_metric = cad_best_train_main_random_metric,
            )
            pred_tv_savename = "{}/mouli_({})_data.json".format(data_args["expt_meta_path"],",".join(topic_subset))
...
def _get_prediction_from_erm(data_args,model_args,data_handler,cat_dataset_full,label_corr_dict,fname_suffix,num_inv_topic,erm_mode=False,cat_dataset_topred=None):
    ...
    best_train_main_metric = None
    best_valid_main_acc = None
    best_valid_main_smin_acc = None
    best_valid_main_pdelta = None 
    best_train_main_random_metric = None 
    ...
    for eidx in range(model_args["epochs"]):
        ...
        if model_args["mouli_valid_sel_mode"]=="loss":
            ...
            if current_valid_metric<best_valid_metric:
                ...
                best_valid_main_smin_acc = float(classifier_main.topic_smin_main_valid_accuracy_list[data_args["debug_tidx"]].result())
                best_valid_main_pdelta = float(classifier_main.topic_flip_main_prob_delta_ldict[data_args["debug_tidx"]]["all"].result())
...
def _get_eiil_environment_labels(reference_classifier,cat_dataset_list,model_args,):
    ...
    for bidx,batch_dict in enumerate(cat_dataset_list):
        ...
        env_prob = np.random.randn(
                                batch_dict["batch"]["label"].shape[0],
        )
        #Now we are ready to create a traininable variable for this batch
        env_prob =tf.Variable(env_prob,dtype=tf.float32)
...
if __name__=="__main__":
    ...
    parser.add_argument('-irm_lambda',dest="irm_lambda",type=float,default=None)
    ...
    model_args["eiil_disc_epoch"]=args.eiil_disc_epoch
    model_args["eiil_disc_lr"]=args.eiil_disc_lr
    model_args["irm_lambda"]=args.irm_lambda
    ...
    if "jtt" in model_args["stage_mode"] or "eiil" in model_args["stage_mode"]:
