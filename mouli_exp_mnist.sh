...
do
    for mouli_yrandom_mode in "train" "notrain"
    do
                    ...
                    do
                        ...
                        do 
                            ...
                            do
                                ...
                                do 
                                    python transformer_debugger.py -expt_num "cad.moulis1mnist.rnum($run_num).topic($topic_name).sample($sample).noise($noise).pvaltsp($pvaltsp).dcf($dcf).mvsel($mvsel_mode).telambda($te_lambda).myrandmode($mouli_yrandom_mode)" -num_sample $sample -num_topics $num_topics -num_epochs $mainepoch -cfactuals_bsize $cbsize -path $path  -out_path $out_path -emb_path "glove-wiki-gigaword-100" -vocab_path "assets/word2vec_10000_200d_labels.tsv" -max_len $max_len  -noise_ratio $noise -num_hidden_layer $hlayer  -main_model_mode $main_mode --normalize_emb -lr $lr -batch_size $batch_size -dtype $dtype -loss_type "x_entropy" -cfactuals_bsize $cbsize  -num_pos_sample $pos_size -num_neg_sample $pos_size  -run_num $run_num -dropout_rate 0.0 -debug_tidx $debug_tidx -stage_mode $stage_mode -sp_topic_pval $pvaltsp -degree_confoundedness $dcf  -topic_name $topic_name -mouli_valid_sel_mode $mvsel_mode  -te_lambda $te_lambda -mouli_yrandom_mode $mouli_yrandom_mode  & #--bert_as_encoder -transformer $transformer --train_bert
                                done
                            done
                        done 
                    done
