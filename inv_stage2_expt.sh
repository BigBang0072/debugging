...
do
    ...
    do
        ...
        do
            for pval in 0.99 0.9 0.8 0.7 0.6 0.5
            do
                python transformer_debugger.py -expt_num "cad.s2.rnum($run_num).noise($noise).pval($pval).erm" -num_sample $sample -num_topics 2 -num_epochs $mainepoch -path "dataset/nlp_toy2/"  -out_path "dataset/nlp_toy2" -emb_path "glove-wiki-gigaword-100" -noise_ratio $noise -num_hidden_layer $hlayer  -main_model_mode $main_mode --normalize_emb -lr 5e-3 -dtype "toynlp2" -loss_type "x_entropy"  -topic0_corr 1.0 -topic1_corr $pval -cfactuals_bsize $cbsize    -run_num $run_num -dropout_rate 0.0 -debug_tidx 1  -stage_mode $stage_mode
            done
        done
    done
done
